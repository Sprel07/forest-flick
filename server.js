...
function buildRaceLevel(game, levelId) {
  ...
  // Simple but nicer levels, can add more later
...
function updateBoulder(game, dt) {
  // boulder item is in game.items with type "boulder"
  ...
  // if boulder hits boss, counts as BOULDER
  if (game.boss) {
...
function stepGame(game, dt) {
  ...
  // update boss independent of turn
  if (game.mode === "boss") {
    updateBoss(game, dt);
    updateBoulder(game, dt);
  }

  // simulate physics when resolving
  if (game.turnState === "resolving" && game.phase === "play") {
    const p = game.players[game.activeId];
    if (!p) return;

    // per tick integration for all players (they can be bumped, coins magnet, etc)
    for (const pid of Object.keys(game.players)) {
      ...
      // stop micro jitter to regain control fast
      ...
      for (const t of game.traps) {
        ...
        if (Math.hypot(dx, dy) <= pl.r + t.r) {
          if (pl.shield) {
            ...
            // reset player slightly (simple punishment)
            pl.x = 120;
            pl.y = game.H / 2;
            pl.vx = 0; pl.vy = 0;
          }
      ...
      // boulder interaction: player bumps it to launch
      ...
      if (b) {
        ...
        if (d <= b.r + pl.r) {
          ...
          // for STUN_THEN_PUNISH, boulder hit counts toward stun if it hits boss later
          // handled in boss logic when boulder hits boss, we can increment stunHits there too
        }
      }

      // boss collisions and damage checks
      if (game.boss) {
        const boss = game.boss;
        ...
        if (dist <= pl.r + boss.r) {
          ...
          if (pl.dashStrikeWindow > 0) {
            ...
            // weakspot body hit if allowed
            // treat normal body hit as small damage only if no restrictive rules
        ...
        // parry ring punish and parry check is triggered by dash event (handled on input)
        // ring hit without dash: punish
    ...
    // end resolving when active player has stopped moving
...
function applyFlick(game, pid, vx, vy) {
  ...
  // Allow dash during this movement if they have charges
  p.canDashThisTurn = true;
  p.dashUsedThisTurn = false;

  // For ricochet rule, we need to track if they bounced at least once before hitting boss.
  // Simple approach: if boss has RICOCHET_REQUIRED, we grant dash strike window only after first wall bounce.
  // We will approximate by allowing dash strike window after they dash, and require ricochet using boss rule on damage source.
}
...
function applyDash(game, pid) {
  ...
  // dash strike window allows boss damage shortly after dash
...
function nextRound(game) {
  ...
  // rotate first player for fairness
...
function ensureRoomLoop(room) {
  ...
  room._loop = setInterval(() => {
    ...
    if (room.lastSnap >= (1 / SNAP_HZ)) {
      ...
      // clear one-time toast after sending
      if (room.game) room.game.toast = "";
    }

    // if round ended, auto advance after short delay
...
wss.on("connection", (ws) => {
  ...
  ws.on("message", (buf) => {
    ...
    if (msg.t === "join") {
      ...
      // send snapshot to this client and notify roster
    ...
    if (msg.t === "act") {
      ...
      if (msg.kind === "dash") {
        // dash only allowed during resolving, but we also allow it while aim if player is already moving from pad or bump
        // for simplicity, allow dash only when resolving
        return;
      }
    }
  ...
  ws.on("close", () => {
    ...
    // if game running, remove player from turn order and players
